...
- Make the responsive design work in a mobile simulator
- Use the [jQuery carousel tutorial](https://github.com/emdubb/jQuery_carousel_tutorial) with the bonus images provided in the [images folder](/starter_code/images/) and turn the header image into a slideshow. To start off with don't worry about adding the header text and button and just try to get the images rotating, but if you get the slideshow working and are feeling ambitious give it a try! *If you attempt this bonus please make sure you commit your homework submission before doing this challenge and then submit this as a seperate commit.* 
...
Students should start by using the [starter code](starter_code/) and adding the proper CSS media queries and CSS to update the site to look good on mobile devices. Students will create a new repository on GitHub called "homework-responsive" and push their code to it when they are ready to submit. Students should also notify the us via Slack once this is done. We will evaluate against the solution code and the student's use of technical elements. We will provide a numeric grade on a scale: does not meet expectations (0); meets expectations (1); exceeds expectations (2).  **Note**: If bullet points have child bullets, grade the child bullets and not the parent bullet belonging to the child. The maximum possible score on this assignment is 28/28. Bonus materials are completely optional.
